import Editor from './core/editor.js';

export default class PetriNetIO {
  /**
   * @param {Object} options
   * @param {string|HTMLElement} options.container - CSS selector or DOM node
   * @param {Object} [options.editorOptions] - forwarded to Editor
   */
  constructor(options = {}) {
    const { container, editorOptions = {}, ...rest } = options;

    if (!container) {
      throw new Error('PetriNetIO: `container` option is required');
    }

    this._container = typeof container === 'string'
      ? document.querySelector(container)
      : container;

    if (!this._container) {
      throw new Error(`PetriNetIO: cannot find container "${container}"`);
    }

    // Editor should be your existing main class
    // Adjust the constructor signature if needed.
    this._editor = new Editor({
      container: this._container,
      ...editorOptions,
      ...rest
    });
  }

  get editor() {
    return this._editor;
  }

  getCanvas() {
    return this._editor.get('canvas');
  }

  getSimulationService() {
    return this._editor.get('simulationService');
  }

  importPNML(pnmlString) {
    const importer = this._editor.get('pnmlImporter'); // adjust if different
    return importer.importPnml(pnmlString);
  }

  loadFromFile() {
    const importer = this._editor.get('pnmlImporter');
    importer.loadFromFile();
  }

  exportPNML(targetFileName = 'model.pnml') {
    const exporter = this._editor.get('pnmlExporter');
    return exporter.exportPnml(targetFileName);
  }

  exportTpn(targetFileName = 'model.tpn') {
    const exporter = this._editor.get('tpnExporter');
    return exporter.exportTpn(targetFileName);
  }

  async exportSVG(targetFileName = 'model.svg') {
    const svgExporter = this._editor.get('svgExporter');
    svgExporter.exportSvg(targetFileName);
  }

  async exportPDF(targetFileName = 'model.pdf') {
    const pdfExporter = this._editor.get('pdfExporter');
    pdfExporter.exportPdf(targetFileName);
  }

  getPetriNet() {
    const canvas = this._editor.get(['canvas']);
    const root = canvas.getRootElement();
    const elements = root.children

    const places = [];
    const transitions = [];
    const connections = [];
    elements.forEach(element => {
        if (element.type === "petri:place") {
            places.push({
              id: element.id,
              tokens: element.businessObject.tokens
            });
        } else if (element.type === "petri:transition") {
          transitions.push({
            id: element.id,
            label: element.businessObject.name
          });
        } else if(element.type === "petri:empty_transition") {
            transitions.push({
              id: element.id
            });
        } else if (element.type === "petri:connection") {
            connections.push({
              source: element.source.id,
              target: element.target.id,
            });
        }
    });
    return {
      places: places,
      transitions: transitions,
      flows: connections
    }
  }

  runAutoLayout(algorithmName = 'sugiyama') {
    const autoLayout = this._editor.get("autoLayoutService");
    autoLayout.run(algorithmName);
  }

  resizePlaces(size) {
    this._resizeElements(size, isPlace);
  }

  resizeTransitions(size) {
    this._resizeElements(size, isTransition);
  }

  _resizeElements(size, predicate) {
    const width = Number(size?.width ?? size);
    const height = Number(size?.height ?? size);

    if (!Number.isFinite(width) || !Number.isFinite(height) || width <= 0 || height <= 0) {
      throw new Error('PetriNetIO: size must be a positive number or { width, height }.');
    }

    const elementRegistry = this._editor.get('elementRegistry');
    const modeling = this._editor.get('modeling');

    elementRegistry.filter(predicate).forEach((element) => {
      modeling.resizeShape(element, {
        x: element.x + (element.width - width) / 2,
        y: element.y + (element.height - height) / 2,
        width,
        height
      });
    });
  }

  destroy() {
    if (this._editor && typeof this._editor.destroy === 'function') {
      this._editor.destroy();
    }

    this._editor = null;
    this._container = null;
  }
}

function isResizablePetriNode(element) {
  return isPlace(element) || isTransition(element);
}

function isPlace(element) {
  return element.type === 'petri:place';
}

function isTransition(element) {
  return element.type === 'petri:transition';
}

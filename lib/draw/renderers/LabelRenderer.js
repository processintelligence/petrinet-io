...
export default class LabelRenderer {
  ...
  render(parentGfx, element) {
    ...
    this._renderAutoLayoutDebugLabel(parentGfx, element);
  ...
  _renderAutoLayoutDebugLabel(parentGfx, element) {
    //TODO remove temporary visual debug label for auto-layout structure detection.
    const label = element.autoLayoutDebugLabel;
    ...
    const y = -8;
    const paddingX = 4;
    const width = label.length * 7 + paddingX * 2;
    const height = 14;

  ...
  extract() {
    ...
    const places = elements
        ...
        .map(place => ({
          id: place.id,
          type: "place",
          name: place.businessObject?.name || "",
          x: place.x,
          y: place.y,
          width: place.width,
          height: place.height,
    ...
    const transitions = elements
        .filter(element => element.type === "petri:transition" || element.type === "petri:empty_transition")
        .map(transition => ({
          id: transition.id,
          type: "transition",
          name: transition.businessObject?.name || "",
          empty: transition.type === "petri:empty_transition",
          x: transition.x,
          y: transition.y,

...
export default class Initialization {
  ...
  initialize(petriNet) {
    ...
    const graphSpacing = this.getGraphSpacing(petriNet.nodes);
    ...
    const vertices = petriNet.nodes.map((node) => {
      ...
      return {
        ...
        impulseX: 0,
        impulseY: 0,
        temperature: Math.min(GEM_CONFIG.maxTemperature, Math.max(GEM_CONFIG.initialTemperature, graphSpacing)),
        skewGauge: 0,
  ...
  buildInsertionPlacement(nodes, degreeById, adjacentById, graphSpacing, random) {
    ...
    for (const node of orderedNodes) {
      ...
      if (placedNeighborCenters.length > 0) {
        ...
        centersById.set(node.id, {
          x: neighborBarycenter.x + Math.cos(angle) * graphSpacing,
          y: neighborBarycenter.y + Math.sin(angle) * graphSpacing
        });
        continue;
      }
      ...
      const angle = random() * 2 * Math.PI;
      const radius = graphSpacing * Math.sqrt(centersById.size + 1);
  ...
  getGraphSpacing(nodes) {
    const nodeDiameters = nodes.map((node) => Math.hypot(
      this.getNodeWidth(node),
      this.getNodeHeight(node)
    ));
    const averageDiameter = nodeDiameters.length > 0
      ? nodeDiameters.reduce((sum, diameter) => sum + diameter, 0) / nodeDiameters.length
      : 0;

    return Math.max(
      GEM_CONFIG.desiredEdgeLength,
      averageDiameter + GEM_CONFIG.nodeGap
    );
  }

export default class StructureTreeBuilder {
  ...
  build(branchRegions, cycleRegions, complexRegions = [], context = null) {
    ...
    for (const complex of complexRegions) {
      const parent = this.findSmallestOverlappingBranch(complex, branches);

      if (parent) {
        parent.children.push(complex);
      }
    }
  ...
  collectExternalPathStartNodeIds(branch, protectedRegion, context) {
    ...
    if (!outgoingByNodeId) {
      return this.uniqueInOrder((branch.pathStartNodeIds || [])
  ...
  findSmallestOwningProtectedRegion(nodeId, protectedRegions) {
    return (protectedRegions || [])
      .filter((region) => this.isProtectedRegionType(region.type))
      .filter((region) => (region.nodeIds || []).includes(nodeId))
      .sort((first, second) =>
        (first.nodeIds || []).length - (second.nodeIds || []).length ||
        this.protectedRegionPriority(first.type) - this.protectedRegionPriority(second.type)
      )[0] || null;
  }

  protectedRegionPriority(type) {
    return type === "cycle" ? 0 : 1;
  }

  isProtectedRegionType(type) {
    return type === "cycle" || type === "complex";
  }
  ...
  expandBranchNodeIdsFromChildren(branch) {
    for (const child of branch.children || []) {
      ...
      branch.nodeIds = this.uniqueSorted([
        ...(branch.nodeIds || []),
        ...(child.nodeIds || [])
      ]);
    }
  ...
  uniqueSorted(nodeIds) {
    return [...new Set(nodeIds)]
      .sort((first, second) => String(first).localeCompare(String(second)));
  }

  uniqueInOrder(nodeIds) {
    return [...new Set(nodeIds)];
  }
}

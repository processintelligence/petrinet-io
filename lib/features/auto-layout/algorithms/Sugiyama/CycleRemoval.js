export default class CycleRemoval {
    ...
    removeCycles(petriNet){
        ...
        const acyclicEdges = petriNet.edges.map((edge) => {
            const isReversed = this.reversedIds.has(edge.id);
        ...
        return{
            ...petriNet,
            edges: acyclicEdges,
            originalEdges: petriNet.edges.map((edge) => ({
                ...edge,
                reversed: this.reversedIds.has(edge.id),
                originalSource: edge.source,
                originalTarget: edge.target
            })),
            feedbackEdgeIds: [...this.reversedIds],
        };
    }

    restoreCycles(petriNet) {
        const originalEdges = Array.isArray(petriNet.originalEdges)
            ? petriNet.originalEdges
            : petriNet.edges;
        ...
        return {
            ...
            edges: originalEdges.map((edge) => {
                const restoredEdge = {
                    ...
                    reversedTemporarily: Boolean(edge.reversed)
                };

                delete restoredEdge.reversed;
                delete restoredEdge.originalSource;
                delete restoredEdge.originalTarget;

export default class CoordinateAssignment {
    ...
    constructor() {
        ...
        this.vertexSep = 40;
        this.layerSizeSpacingFactor = 0.35;
        this.vertexSizeSpacingFactor = 0.2;
    ...
    assignCoordinates(petriNet) {
        ...
        const layerLeftByIndex = this.computeLayerLefts(petriNet.nodes);
        ...
        petriNet.nodes = petriNet.nodes.map((node) => {
            ...
            const layerLeft = layerLeftByIndex.get(layer) ?? this.startX;
            const layerWidth = this.getLayerWidth(petriNet.nodes, layer);
            const nodeWidth = this.getNodeWidth(node);

            return {
                ...node,
                x: layerLeft + (layerWidth - nodeWidth) / 2,
                y: top + verticalShift
            };
        });
    ...
    computeLayerLefts(nodes) {
        const maxLayer = this.getMaxLayer(nodes);
        const leftByLayer = new Map();
        let cursor = this.startX;

        for (let layer = 0; layer <= maxLayer; layer++) {
            leftByLayer.set(layer, cursor);
            cursor += this.getLayerWidth(nodes, layer) + this.getLayerGap(nodes, layer, layer + 1);
        }

        return leftByLayer;
    }

    getLayerGap(nodes, layer, nextLayer) {
        const layerWidth = this.getLayerWidth(nodes, layer);
        const nextLayerWidth = this.getLayerWidth(nodes, nextLayer);

        if (nextLayerWidth === 0) {
            return this.layerSpacing;
        }

        return this.layerSpacing + (layerWidth + nextLayerWidth) * this.layerSizeSpacingFactor;
    }

    getMaxLayer(nodes) {
        const layerValues = nodes
            .map((node) => Number.isFinite(node.layer) ? node.layer : 0)
        ...
        return Math.max(...layerValues);
    }

    getLayerWidth(nodes, layer) {
        const layerWidths = nodes
            .filter((node) => (Number.isFinite(node.layer) ? node.layer : 0) === layer)
    ...
    createInitialCenters(layers) {
        ...
        for (const layer of layers) {
            ...
            cursor = -((layerHeights + layerGaps) / 2);
    ...
    getMinimumSeparation(a, b) {
        const combinedHeight = this.getNodeHeight(a) + this.getNodeHeight(b);

        return combinedHeight / 2 + this.vertexSep + combinedHeight * this.vertexSizeSpacingFactor;
    }

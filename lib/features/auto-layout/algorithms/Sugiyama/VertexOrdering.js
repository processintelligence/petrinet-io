export default class VertexOrdering {
    ...
    orderVertex(petriNet) {
        ...
        let bestLayers = this.copyLayers();
        const maxIterations = 10;

        for (let iteration = 0; iteration < maxIterations; iteration++) {
            this.weightedMedian(iteration);
            this.transpose();
            const currentCrossings = this.countTotalCrossings();

            if (currentCrossings < bestCrossings) {
                bestCrossings = currentCrossings;
                bestLayers = this.copyLayers();
            }
        }

        this.layeredNodes = bestLayers;
    ...
    downwardSweep(){
        for (let layerIndex = 1; layerIndex < this.layeredNodes.length; layerIndex++) {
            ...
            this.sortLayerByMedian(currentLayer);
        }
    }
    ...
    sortLayerByMedian(layer) {
        layer.sort((a, b) => {
            const medianA = this.medianByNodeId.get(a.id);
            const medianB = this.medianByNodeId.get(b.id);
    ...
    transpose() {
        ...
        while (improved) {
            ...
            for (let layerIndex = 0; layerIndex < this.layeredNodes.length; layerIndex++) {
                const layer = this.layeredNodes[layerIndex];

                if (!layer || layer.length < 2) {
                    continue;
                }

                for (let nodeIndex = 0; nodeIndex < layer.length - 1; nodeIndex++) {
                    const originalCrossings = this.countCrossingsAroundLayer(layerIndex);

                    this.swapLayerNodes(layerIndex, nodeIndex, nodeIndex + 1);

                    const swappedCrossings = this.countCrossingsAroundLayer(layerIndex);

                    if (swappedCrossings < originalCrossings) {
                        improved = true;
                        continue;
                    }
    ...
    swapLayerNodes(layerIndex, leftIndex, rightIndex) {
        const layer = this.layeredNodes[layerIndex];
        const temp = layer[leftIndex];

        layer[leftIndex] = layer[rightIndex];
        layer[rightIndex] = temp;
    }

    countCrossingsAroundLayer(layerIndex) {
        let crossings = 0;

        if (layerIndex > 0) {
            crossings += this.countCrossingsBetweenLayers(
                this.layeredNodes[layerIndex - 1],
                this.layeredNodes[layerIndex]
            );
        }

        if (layerIndex < this.layeredNodes.length - 1) {
            crossings += this.countCrossingsBetweenLayers(
                this.layeredNodes[layerIndex],
                this.layeredNodes[layerIndex + 1]
            );

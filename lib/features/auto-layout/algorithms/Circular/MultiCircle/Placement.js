export default class Placement {
  ...
  measureSubtree(blockId, blockById, blockLayoutById, parentInfoByBlockId) {
    ...
    const radius = this.getBlockOuterRadius(
      blockLayoutById.get(blockId),
  ...
  buildChildTransform(placement, blockLayoutById) {
    ...
    const minimumDistance = placement.childMetric.radius + this.blockGap;
    const distanceFromAnchor = Math.hypot(
      center.x - placement.anchor.x,
      center.y - placement.anchor.y
    );

    return {
      center: distanceFromAnchor < minimumDistance
        ? this.pointFrom(placement.anchor, placement.angle, minimumDistance)
        : center,
      rotation
    };
  }
  ...
  getBlockOuterRadius(blockLayout, block) {
    if (!blockLayout?.circularCenter || !Array.isArray(blockLayout.nodes) || blockLayout.nodes.length === 0) {
      return this.getFallbackRadius(block);
    }

    const center = blockLayout.circularCenter;
    const radius = Math.max(...blockLayout.nodes.flatMap((node) => {
      const left = Number.isFinite(node.x) ? node.x : 0;
      const top = Number.isFinite(node.y) ? node.y : 0;
      const right = left + this.getNodeWidth(node);
      const bottom = top + this.getNodeHeight(node);

      return [
        Math.hypot(left - center.x, top - center.y),
        Math.hypot(right - center.x, top - center.y),
        Math.hypot(right - center.x, bottom - center.y),
        Math.hypot(left - center.x, bottom - center.y)
      ];
    }));

    return Math.max(this.getFallbackRadius(block), radius);
  }

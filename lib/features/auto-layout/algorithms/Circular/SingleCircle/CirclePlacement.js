export default class CirclePlacement {
  ...
  place(petriNet) {
    ...
    this.getSizeAwareAngles(circularOrder, nodeById, nodeGap).forEach(({ nodeId, angle }) => {
  ...
  getSizeAwareAngles(circularOrder, nodeById, nodeGap) {
    ...
    const arcLengths = circularOrder.map((nodeId, index) => {
      const node = nodeById.get(nodeId);
      const nextNodeId = circularOrder[(index + 1) % circularOrder.length];
      const nextNode = nodeById.get(nextNodeId);

      return node && nextNode
        ? this.getCenterArcLength(node, nextNode, nodeGap)
        : nodeGap;
    });
    const totalArcLength = arcLengths.reduce((sum, arcLength) => sum + arcLength, 0);

    if (!Number.isFinite(totalArcLength) || totalArcLength <= 0) {
      const angleStep = (2 * Math.PI) / circularOrder.length;

      return circularOrder.map((nodeId, index) => ({
        nodeId,
    ...
    circularOrder.forEach((nodeId, index) => {
      ...
      if (index === circularOrder.length - 1) {
        return;
      }

      angle += (arcLengths[index] / totalArcLength) * 2 * Math.PI;
    });

...
export default class TransitionParser {
    ...
    static parse(transitionNode, elementFactory, canvas, root, defaultPnml, findLabel) {
        ...
        const colors = parseColors(transitionNode);
...
function parseColors(node) {
    const backgroundNode = node.querySelector('toolspecific[tool="petrinet.io"] > property[key="backgroundColor"]');
    const borderNode = node.querySelector('toolspecific[tool="petrinet.io"] > property[key="borderColor"]');
    const colors = {};

    if (backgroundNode) {
        colors.fill = backgroundNode.getAttribute('value');
    }

    if (borderNode) {
        colors.stroke = borderNode.getAttribute('value');
    }
